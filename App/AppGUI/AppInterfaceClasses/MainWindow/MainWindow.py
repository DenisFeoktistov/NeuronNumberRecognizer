...
from App.AppGUI.AppInterfaceClasses.MainWindow.MainWindowResponder import MainWindowResponder
from SubsidiaryFiles.Classes.NeuronWidget.NeuronWidget import NeuronWidget

import App.AppGUI.AppInterfaceClasses.MainAppInterface as MainAppInterface
import \
    App.AppGUI.AppInterfaceClasses.MainWindow.ModeWindows.AutoTrainingModeWindow.AutoTrainingModeWindow as TrainingModeWindow
import App.AppGUI.AppInterfaceClasses.MainWindow.ModeWindows.ManualTestingModeWindow.ManualTestingModeWiindow as \
...
class MainWindow(QMainWindow):
    ...
    def add_batches_label(self) -> None:
        ...
        font_size = self.height // 50
        self.batches_text_label.move(self.name_text_label.x(), self.name_text_label.y() + self.name_text_label.height())
        self.batches_text_label.resize(self.name_text_label.width(), self.name_text_label.height())
        self.batches_text_label.setStyleSheet(f"font-size: {font_size}px; color: black")

        self.batches_label = QLabel(parent=self)
        font_size = self.height // 50
        self.batches_label.move(self.batches_text_label.x() + self.batches_text_label.width(),
